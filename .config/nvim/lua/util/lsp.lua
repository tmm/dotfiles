...
function M.rename_file()
	...
	local old = assert(require("util.root").realpath(vim.api.nvim_buf_get_name(buf)))
	local root = assert(require("util.root").realpath(require("util.root").get({ normalize = true })))
	...
	vim.ui.input({
		...
		new = require("util.init").norm(root .. "/" .. new)
...
function M.formatter(opts)
	...
	local ret = {
		...
		priority = 1,
		format = function(buf)
			M.format(require("util.init").merge({}, filter, { bufnr = buf }))
		end,
...
function M.format(opts)
	opts = vim.tbl_deep_extend(
		...
		{},
		opts or {},
		require("util.init").opts("nvim-lspconfig").format or {},
		require("util.init").opts("conform.nvim").format or {}
	)
			...
			callback = function(ev)
				...
				if not ({ M.words.get() })[2] then
					if ev.event:find("CursorMoved") then
						vim.lsp.buf.clear_references()
					elseif not require("util.cmpx").visible() then
