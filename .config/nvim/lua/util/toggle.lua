...
function M.wrap(toggle)
  return setmetatable(toggle, {
    __call = function()
      ...
      if state then
        require("util.init").info("Enabled " .. toggle.name, { title = toggle.name })
      else
        require("util.init").warn("Disabled " .. toggle.name, { title = toggle.name })
      end
      return state
    end,
  }) --[[@as Toggle.wrap]]
...
function M.map(lhs, toggle)
  ...
  require("util.init").safe_keymap_set("n", lhs, function()
    t()
...
function M.wk(lhs, toggle)
  if not require("util.init").has("which-key.nvim") then
    return
  end
  local function safe_get()
    ...
    if not ok then
      require("util.init").error(
        { "Failed to get toggle state for **" .. toggle.name .. "**:\n", enabled },
        { once = true }
      )
    end
    return enabled
...
function M.format(buf)
  return M.wrap({
    ...
    get = function()
      ...
      return require("util.format").enabled()
    end,
    set = function(state)
      require("util.format").enable(state, buf)
    end,
  })
end
  ...
  set = function(state)
    if state then
      ...
      vim.api.nvim_create_autocmd("ExitPre", {
        once = true,
        group = vim.api.nvim_create_augroup("tmm_restore_max_exit_pre", { clear = true }),
