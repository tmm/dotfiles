...
function M.detectors.lsp(buf)
	...
	for _, client in pairs(require("util.lspx").get_clients({ bufnr = buf })) do
	...
	return vim.tbl_filter(function(path)
		path = require("util.init").norm(path)
...
function M.realpath(path)
	...
	return require("util.init").norm(path)
end
...
function M.info()
	...
	lines[#lines + 1] = "```"
	require("util.init").info(lines, { title = "Roots" })
...
function M.get(opts)
	...
	return require("util.init").is_win() and ret:gsub("/", "\\") or ret
end
