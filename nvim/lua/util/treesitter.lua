...
function M.ensure_treesitter_cli(cb)
  ...
  mr.refresh(function()
    ...
    if not p:is_installed() then
      require("util.init").info("Installing `tree-sitter-cli` with `mason.nvim`...")
      p:install(
        nil,
        vim.schedule_wrap(function(success)
          if success then
            require("util.init").info("Installed `tree-sitter-cli` with `mason.nvim`.")
            cb(true)
          else
