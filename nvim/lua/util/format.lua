...
function M.info(buf)
  ...
  require("util.init")[enabled and "info" or "warn"](
    table.concat(lines, "\n"),
...
function M.format(opts)
  ...
  for _, formatter in ipairs(M.resolve(buf)) do
    if formatter.active then
      ...
      require("util.init").try(function()
  ...
  if not done and opts and opts.force then
    require("util.init").warn("No formatter available", { title = "Format" })
  end
end
...
function M.setup()
  ...
  -- Manual format
  vim.api.nvim_create_user_command("TmmFormat", function()
    M.format({ force = true })
  end, { desc = "Format selection or buffer" })

  -- Format info
  vim.api.nvim_create_user_command("TmmFormatInfo", function()
    M.info()
